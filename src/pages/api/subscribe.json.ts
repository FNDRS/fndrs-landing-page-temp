...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (sendResend.data) {
      return new Response(JSON.stringify({ message: "Message successfully sent!" }), { status: 200, statusText: "OK" })
    }
    ...
    return new Response(JSON.stringify({ message: "Error connecting to the server." }), {
      status: 500,
